...
function initInputScene() {
    ...
    inputRenderer.setClearColor(0x000000, 1);
    ...
    var urls = [], urls2 = [];
    for (var i = 0; i < 6; i++) {
        var url = "../tex/stripe8.jpg", url2 = "../tex/stripe9.jpg";
        urls.push(url);
        urls2.push(url2);
    }
    ...
    for(var j = 0; j<2; j++){
        for(var i = 1; i < 25; i++){
            loadModel("../shards/"+i+".js", Math.random()*w - w/2, Math.random()*h - h/2, 100,1000.0,0,0,0, dazzleMaterial);
        }
        for(var i = 25; i < 50; i++){
            loadModel("../shards/"+i+".js", Math.random()*w - w/2, Math.random()*h - h/2, 100,1000.0,0,0,0, dazzleMaterial2);
        }
    }
...
function initFrameDifferencing() {
    ...
    "syrup-fs"); //string for fragment shader id - the only lines that really matter in this function, or the only lines you'll wanna change
